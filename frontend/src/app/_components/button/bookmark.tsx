...
export default function BookmarkBtn() {
    ...
    const removeBm = (key: any) => {
        ...
        if (storedBookmarks) {
            ...
            const existingBm = parsedBookmarks.findIndex(bm => getHref(bm.url) === key)
                            ...
                            <ModalBody>
                                <ul className="max-h-[400px] overflow-y-auto bmark-cont flex flex-col my-4">
                                    {bookmarks && bookmarks.length > 0 ? bookmarks.map((node, idx) => {
                                        const hashedHref = getHref(node.url)
                                        console.log('[bm nav btn] hashedHref', hashedHref)
                                        return (
                                            <li key={`sr${node.url}-${idx}`} className="flex p-2 justify-center items-center relative flex-col">
                                                <Link href={hashedHref} isDisabled={hashedHref === pathname ? true : false} isBlock className="flex w-full">
                                                ...
                                                <Button size="lg" variant="light" color="default" onPress={() => removeBm(hashedHref)} className="w-full text-sm text-default-500 mt-4" aria-label={`delete bookmark of ${node.name}`} startContent={<MdDelete size={20}></MdDelete>}>Delete this bookmark</Button>
